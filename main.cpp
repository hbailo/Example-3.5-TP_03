...
int main()
{
  inputsInit();  // Llamada a función
  outputsInit(); // Llamada a función
  while (true) { 
    alarmActivationUpdate();	// Llamada a función
    alarmDeactivationUpdate();  // Llamada a función
    uartTask();			// Llamada a función
    delay(TIME_INCREMENT_MS);   // Llamada a función
    
    printf("Gas detector (MQ2) = %d | User buttons code = %X | Alarm test button = %d | Enter button = %d\n", mq2.read(), userButtons.read(), alarmTestButton.read(), enterButton.read()) ;
    printf("Potentiometer = %f [V] | LM35 = %f\n", potentiometer.read_voltage(), lm35.read()) ;
    printf("Alarm LED = %d | Incorrect code LED = %d | System blocked LED = %d | Siren Pin = %d\n", alarmLed.read(), incorrectCodeLed.read(), systemBlockedLed.read(), sirenPin.read()) ;
  }
}
